...
public class RayIntersectionTest
{
    ...
    {
        Ray ray = new Ray(new MutableVec2f(0.1f, 0.1f), new MutableVec2f(0, 1));
        Circle circle = new Circle(1, 1, 1);
        Intersection intersec = circle.intersect(ray);
        assertTrue(intersec.isHit());
        ray = new Ray(new MutableVec2f(0, -2), new MutableVec2f(1, 1));
        intersec = circle.intersect(ray);
        assertTrue(!intersec.isHit());
        ray = new Ray(new MutableVec2f(-1, -1f), new MutableVec2f(1.5f, 1));
        intersec = circle.intersect(ray);
        assertTrue(intersec.isHit());
        ray = new Ray(new MutableVec2f(2, 2), new MutableVec2f(0, 1));
        intersec = circle.intersect(ray);
        assertTrue(intersec.isHit());
    }
}
