...
public final class CSVFormat implements Serializable
{
     ...
     */
    public static final CSVFormat DEFAULT             = new CSVFormat(COMMA, DOUBLE_QUOTE_CHAR, null, null, null, false,
            true, CRLF, null, null, null, false, false, false, false, false);
     ...
     * {@code UNLOAD TO file_name} operation.
     *
     * <p>
     * This is a comma-delimited format with a LF character as the line
     * separator. Values are not quoted and special characters are escaped with
     * {@code '\'}. The default NULL string is {@code "\\N"}.
     * </p>
     *
     * <p>
     * Settings are:
     ...
     *      http://www.ibm.com/support/knowledgecenter/SSBJG3_2.5.0/com.ibm.
     *      gen_busug.doc/c_fgl_InOutSql_UNLOAD.htm</a>
     * @since 1.3
     */
    ...
    /**
     * Default Informix CSV UNLOAD format used by the
     * {@code UNLOAD TO file_name} operation (escaping is disabled.)
     *
     * <p>
     * This is a comma-delimited format with a LF character as the line
     * separator. Values are not quoted and special characters are escaped with
     ...
     * Values are not quoted and special characters are escaped with {@code '\'}
     ...
     */
    public static final CSVFormat MYSQL               = DEFAULT.withDelimiter(TAB).withEscape(BACKSLASH)
            .withIgnoreEmptyLines(false).withQuote(null).withRecordSeparator(LF).withNullString("\\N");
     ...
     * fields but the delimiter will be initialized with null/false.
     * </p>
     *
     * @param delimiter
    ...
    private final Character commentMarker;           // null if commenting is
                                                     // disabled

    private final char      delimiter;

    private final Character escapeCharacter;         // null if escaping is
                                                     // disabled

    private final String[]  header;                  // array of header column
                                                     // names

    private final String[]  headerComments;          // array of header comment
                                                     // lines

    private final boolean   ignoreEmptyLines;

    private final boolean   ignoreHeaderCase;        // should ignore header
                                                     // names case
    ...
    private final String    nullString;              // the string to be used
                                                     // for null values

    private final Character quoteCharacter;          // null if quoting is
                                                     // disabled
    ...
    public String format(final Object... values)
    {
        ...
        try
        {
            CSVPrinter p = new CSVPrinter(out, this);
     ...
     *         {@code false} if they are turned into empty records.
     */
     ...
     *
     * @return {@code true} if header names cases are ignored, {@code false} if
     ...
     * The value will be escaped or encapsulated as needed. Useful when one
     * wants to avoid creating CSVPrinters.
     *
     * @param value
     *            value to output.
     * @param out
     ...
     *
     * <p>
     * The values will be quoted if needed. Quotes and new-line characters will
     * be escaped. This method adds the record separator to the output after
     * printing the record, so there is no need to call
     * {@link #println(Appendable)}.
     * </p>
     *
     * @param out
     *            where to write
    ...
    /**
     * Returns a new {@code CSVFormat} with the missing column names behavior of
     ...
     *            missing column names in the header line, {@code false} to
     ...
     * </pre>
     *
     * @return A new CSVFormat that is equal to this but using the first record
     *         as header.
    ...
    /**
     * Returns a new {@code CSVFormat} with the header of the format defined by
     * the enum class.
     *
     * <p>
     * Example:
     ...
     *
     * @param headerEnum
     *            the enum defining the header, {@code null} if disabled, empty
     *            if parsed automatically, user specified otherwise.
     *
     * @return A new CSVFormat that is equal to this but with the specified
     *         header
     * @see #withHeader(String...)
     ...
     * the result set metadata. The header can either be parsed automatically
     ...
     *
     * @param resultSet
     *            the resultSet for the header, {@code null} if disabled, empty
     *            if parsed automatically, user specified otherwise.
     *
     * @return A new CSVFormat that is equal to this but with the specified
     *         header
     ...
     * given values. The header can either be parsed automatically from the
     * input file with:
     *
     * <pre>
     * CSVFormat format = aformat.withHeader();
     ...
     * set to the given values. The comments will be printed first, before the
     * headers. This setting is ignored by the parser.
     *
     * <pre>
     ...
     *
     * @param headerComments
     *            the headerComments which will be printed by the Printer before
     *            the actual CSV data.
     *
     * @return A new CSVFormat that is equal to this but with the specified
     *         header
    ...
    /**
     * Returns a new {@code CSVFormat} with the empty line skipping behavior of
     * the format set to {@code true}.
     *
     * @return A new CSVFormat that is equal to this but with the specified
     ...
     *            empty lines between the records, {@code false} to translate
    ...
    /**
     * Returns a new {@code CSVFormat} with the header ignore case behavior set
     * to {@code true}.
     *
     ...
     *
     * @param ignoreHeaderCase
     *            the case mapping behavior, {@code true} to access name/values,
     ...
     *
     * @param ignoreSurroundingSpaces
     *            the trimming behavior, {@code true} to remove the surrounding
     ...
     *
     * <p>
     * <strong>Note:</strong> This setting is only used during printing and does
     * not affect parsing. Parsing currently only works for inputs with '\n',
     * '\r' and "\r\n"
     * </p>
     *
     * @param recordSeparator
